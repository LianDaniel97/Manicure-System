...
$(document).ready(function () {
    if (typeof AppointmentManager === 'undefined') {
        $('main .container').first().prepend('<div style="background: #f8d7da; color: #721c24; padding: 15px; border: 1px solid #f5c6cb; border-radius: 5px; margin-bottom: 20px; text-align: center; direction: rtl;"><strong>שגיאה critical:</strong> מערכת הניהול (appointment_manager.js) לא נטענה. ייתכן שיש בעיה בשם הקובץ או בנתיב.</div>');
        console.error('CRITICAL: AppointmentManager class is not defined. appointment_manager.js failed to load.');
        return;
    }
